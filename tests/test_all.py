"""All-tests for df-mathematik-der-macht-writer (consolidated). [CRUX-MK]
...
from src.all_modules import (
    OutlineManager, Chapter, MATHEMATIK_DER_MACHT_OUTLINE,
    StyleGuide, ChapterGenerator, AdapterOrchestrator)


def test_outline_manager_init():
    om = OutlineManager()
    assert om.book_title == "Mathematik der Macht"


def test_chapter_count_is_16():
    """Welle-44 spec: 16 Kapitel."""
    assert OutlineManager().chapter_count() == 16
...
def test_style_guide_academic_max_words():
    """Academic style allows longer sentences (35)."""
    sg = StyleGuide()
    assert sg.max_sentence_words == 35


def test_audit_short_text_passes():
    sg = StyleGuide()
    text = "Macht ist mathematisch. Hamilton zeigt das. Klar."
...
def test_audit_long_sentence_warns():
    ...
    text = "Dies ist ein sehr langer akademischer Satz."
    report = sg.audit(text)
    assert any(v.rule == "academic_sentence_length" for v in report.violations)
...
def test_generator_init():
    ...
    assert gen.book_title == "Mathematik der Macht"
...
def test_real_mode_with_ticket_raises_not_implemented():
    with patch.dict(os.environ, {
        ...
        "PHRONESIS_TICKET": "PT-2026-05-11-003",
    }, clear=False):
        gen = ChapterGenerator()
        try:
            gen.generate_chapter(Chapter(1, "X"))
...
def test_orchestrator_init():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert orch.book_title == "Mathematik der Macht"
...
def test_run_with_specific_chapters():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = orch.run(chapter_numbers=[1, 5, 10])
        assert result.chapters_generated == 3
